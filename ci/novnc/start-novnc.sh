...
start_pretty_desktop() {
	...
	# in start_pretty_desktop()
	command -v setxkbmap >/dev/null 2>&1 &&
	DISPLAY="$DISPLAY" setxkbmap -layout us >/dev/null 2>&1 || true
...
distro_start_vnc_server() {
	...
	if command -v x11vnc >/dev/null 2>&1; then
		...
		if [ -n "${X11VNC_PASSWORD:-}" ]; then
			...
			x11vnc  -storepasswd "$X11VNC_PASSWORD" "$passfile" >/dev/null 2>&1
			pass_opt="-rfbauth $passfile"
		fi
		if ! pgrep -x x11vnc >/dev/null 2>&1; then
			log  "Launching x11vnc on ${VNC_BIND}:${VNC_PORT}"
			x11vnc  -display "$DISPLAY" -rfbport "$VNC_PORT" $bind_opt -forever -shared $pass_opt -bg -quiet >/dev/null 2>&1
		fi
		wait_for_vnc
		return
	fi
